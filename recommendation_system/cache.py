"""Redis-backed cache singleton for the standalone recommendation service.

This is a verbatim-logic copy of ``backend/app/services/cache.py`` with the
single change that it imports from ``recommendation_system.config`` instead of
``app.config``.  The ``RedisCache`` class and ``cache`` singleton are
identical in behaviour.

When the recommendation_system package is mounted *inside the monolith* the
services import ``from app.services.cache import cache`` (via the conditional
import pattern).  This module is only used by the standalone service process.

Key-namespace convention (same as the monolith):
  mb:reco:mood:{mood}:{limit}:u:{user_or_anon}   – mood recommendation lists
  mb:reco:foryou:{user}:{limit}                  – personalised feed
  mb:taste:{user}                                – taste vector
  mb:user:lastplayed:{user}                      – last-played song IDs
  mb:user:mostplayed:{user}                      – most-played song IDs
  mb:user:icount:{user}                          – interaction count
  mb:pop:{type}:{song}                           – popularity counters
...
class RedisCache:
    ...
    once per error-kind and return a safe default so callers never need to
    wrap every call in try/except.
    """

    def __init__(self, url: Optional[str] = None) -> None:
        self._url = url or _settings.REDIS_URL
        self._enabled = _settings.CACHE_ENABLED and Redis is not None
...
# Import as ``from recommendation_system.cache import cache`` in standalone mode.
cache = RedisCache()
