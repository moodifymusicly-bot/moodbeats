...
# 1. PostgreSQL setup for Arch Linux
...
sleep 2
su - postgres -c "psql -tc \"SELECT 1 FROM pg_roles WHERE rolname='${PG_USER}'\"" | grep -q 1 || su - postgres -c "psql -c \"CREATE USER ${PG_USER} WITH PASSWORD '${PG_PASS}'\""
su - postgres -c "psql -tc \"SELECT 1 FROM pg_database WHERE datname='${PG_DB}'\"" | grep -q 1 || su - postgres -c "psql -c \"CREATE DATABASE ${PG_DB} OWNER ${PG_USER}\""
...
sed -i "s/^requirepass.*/requirepass ${REDIS_PASS}/" "${REDIS_CONF}"
if ! grep -q "^requirepass" "${REDIS_CONF}"; then echo "requirepass ${REDIS_PASS}" >> "${REDIS_CONF}"; fi
...
python -m venv recommendation_system/.venv
recommendation_system/.venv/bin/pip install -q -r recommendation_system/requirements.txt
...
command=${MOODBEATZ_DIR}/backend/.venv/bin/gunicorn app.main:app --bind 127.0.0.1:8001 --workers 4 --worker-class uvicorn.workers.UvicornWorker --timeout 120
...
command=${MOODBEATZ_DIR}/recommendation_system/.venv/bin/gunicorn recommendation_system.main:app --bind 127.0.0.1:8002 --workers 2 --worker-class uvicorn.workers.UvicornWorker --timeout 120
...
DATABASE_URL="postgresql+asyncpg://${PG_USER}:${PG_PASS}@127.0.0.1:5432/${PG_DB}" "${MOODBEATZ_DIR}/backend/.venv/bin/python" -m alembic upgrade head
