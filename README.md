...
- YouTube Data API v3 key (recommended; free tier is enough — without it, curated fallback videos are used)
...
| `YOUTUBE_API_KEY`                     | backend        | recommended (curated fallback if unset) |
...
- `GET /api/health` - liveness + DB probe; includes `youtube_data_api_configured`.
...
fork.
- **Before `docker compose build` on the server**, set `.env` so the
frontend bakes the correct API origin: `NEXT_PUBLIC_API_URL` must be the
public URL users reach (for example `https://your-domain.com` or
`https://api.your-domain.com` if the API is on a subdomain). Match
