...
export default function MoodPlaylist() {
  ...
  // Mood-matched tracks first, padded with the rest for a 30-track feel
  const matched = MOCK_TRACKS.filter((t) => t.moods.includes(mood));
  const others = MOCK_TRACKS.filter((t) => !t.moods.includes(mood));
  const playlist: Track[] = [...matched, ...others].slice(0, 12);

  const totalSeconds = playlist.reduce((sum, t) => sum + t.duration, 0);
